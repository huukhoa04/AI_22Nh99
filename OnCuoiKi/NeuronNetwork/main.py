...
# Load training data
def load_training_data(csv_path, num_classes):
...
if __name__ == "__main__":
    ...
    print("NN")
    # test NN
    num_classes = 3  # Example: 3 classes (0,1,2)
    input_features = 3
    layers = NN([input_features, 4, 5, num_classes])
    # Create some sample training data
    training_data = []
    training_data = load_training_data('data.csv', num_classes)

    # for _ in range(100):
    #     # Generate random input features
    #     x = np.random.randn(3, 1)  # 3 input features
    #     # Create target output (simple binary classification)
    #     y = np.zeros((2, 1))
    #     y[1 if x[0] > 0 else 0] = 1  # Class based on first feature
    #     training_data.append((x, y))

    # Create static training data for reproducible results
    # np.random.seed(42)  # Set random seed
    # for i in range(100):
    #     x = np.array([[0.1 * i], [0.2 * i], [0.3 * i]])
    #     y = np.zeros((2, 1))
    #     y[1 if x[0] > 1.5 else 0] = 1
    #     training_data.append((x, y))
    ...
    costs = layers.train(training_data, epochs, mini_batch_size, learning_rate)

    # Print final cost
    print(f"Final cost: {costs[-1]}")

    # Test prediction
    # random generated expected output
    test_input = np.random.randn(3, 1)
    # static expected output
    # test_input = np.array([[0.1], [0.2], [0.3]])

    prediction = layers.predict(test_input)
    print(f"Prediction for {test_input.flatten()}: {prediction}")
    # data
